        ...
        <div class="p-6">
            ...
            <div class="mb-4 flex flex-col md:flex-row justify-between gap-4">
                <div class="relative flex-1 max-w-md">
                    <form method="GET" action="{{ route('owner.dashboard') }}" class="relative">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <select onchange="window.location.href=this.value" 
                        ...
                        @endphp
                        <option value="{{ route('owner.dashboard', array_merge(request()->query(), ['per_page' => 5])) }}" {{ $perPage == 5 ? 'selected' : '' }}>5</option>
                        <option value="{{ route('owner.dashboard', array_merge(request()->query(), ['per_page' => 10])) }}" {{ $perPage == 10 ? 'selected' : '' }}>10</option>
                        <option value="{{ route('owner.dashboard', array_merge(request()->query(), ['per_page' => 50])) }}" {{ $perPage == 50 ? 'selected' : '' }}>50</option>
                    </select>
                </div>
            </div>

...
<div class="space-y-6" x-data="customerManager()">
    ...
    <div class="relative max-w-md">
        <form method="GET" action="{{ route('owner.customers.index') }}" class="relative">
            ...
            @if(request('search'))
                <a href="{{ route('owner.customers.index') }}" class="absolute right-3 top-3 text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></a>
            @endif
        </form>
    </div>
                ...
                <form method="POST" :action="form.id ? updateUrl : storeUrl" id="customerForm">
                    ...
                    <div class="px-6 pt-6 pb-4">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            {{-- Alamat --}}
                            <div>
                                <label class="block text-sm font-medium mb-1" :class="darkMode ? 'text-gray-300' : 'text-gray-700'">Alamat</label>
                                <textarea name="address" x-model="form.address" rows="2" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37]" :class="darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'"></textarea>
                            </div>
                        </div>
                    </div>
...
<script>
    function customerManager() {
        return {
            ...
            modalTitle: '',
            modalSubmitText: '',
            form: { id: null, name: '', email: '', phone: '', address: '', total_visits: 0, last_visit: '' },
            storeUrl: storeUrl,
            get updateUrl() {
                return this.form.id ? updateBaseUrl.replace('__ID__', this.form.id) : '';
            },
            resetForm() {
                this.form = { id: null, name: '', email: '', phone: '', address: '', total_visits: 0, last_visit: '' };
            },
            openCreateModal() {
                this.resetForm();

...
<div class="space-y-6" x-data="userManager()">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-xl font-semibold" :class="darkMode ? 'text-white' : 'text-gray-800'">Kelola Users</h2>
        ...
        @forelse($users as $user)
            <div class="rounded-lg shadow p-4 flex items-center justify-between" :class="darkMode ? 'bg-gray-800' : 'bg-white'">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button @click="openEditModal({{ $user }})" class="text-gray-400 hover:text-gray-600">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button @click="confirmDelete({{ $user->id }}, '{{ addslashes($user->username) }}')"
                            class="transition"
                    ...
                    <div class="px-6 pt-6 pb-4">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input :type="showPassword ? 'text' : 'password'" name="password" x-model="form.password"
                                        class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent pr-10"
                                        :class="darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'"
                                        :required="!form.id">
                            ...
                            <div>
                                ...
                                <p x-show="isStatusDisabled" class="text-xs mt-1 text-yellow-500">
                                    Status tidak dapat diubah untuk user ini.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    function userManager() {
        return {
            ...
            get isStatusDisabled() {
                // Nonaktifkan jika user adalah owner, atau jika sedang edit diri sendiri (admin)
                return (this.form.id && this.form.role === 'owner') || (this.form.id && this.form.id == currentUserId);
            },
            togglePassword() {

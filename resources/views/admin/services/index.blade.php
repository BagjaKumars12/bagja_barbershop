...
<div class="space-y-6" x-data="serviceManager()">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-xl font-semibold" :class="darkMode ? 'text-white' : 'text-gray-800'">Kelola Service</h2>
        ...
        <button @click="openCreateModal()" class="px-4 py-2 bg-[#D4AF37] text-white rounded-lg hover:bg-[#b8942f] transition flex items-center space-x-2">
            <i class="fas fa-plus"></i><span>Tambah Service</span>
        </button>
    </div>
            ...
            <tbody class="divide-y" :class="darkMode ? 'divide-gray-700' : 'divide-gray-200'">
                @forelse($services as $index => $service)
                    <tr :class="darkMode ? 'bg-gray-800' : 'bg-white'">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <button @click="openEditModal({{ $service }})" class="text-gray-400 hover:text-gray-600 mr-2">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button @click="confirmDelete({{ $service->id }}, '{{ addslashes($service->name) }}')" class="text-gray-400 hover:text-red-600">

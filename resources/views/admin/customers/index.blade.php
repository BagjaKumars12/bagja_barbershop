...
<div class="space-y-6" x-data="customerManager()">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-xl font-semibold" :class="darkMode ? 'text-white' : 'text-gray-800'">Kelola Customers</h2>
    ...
    <div class="relative max-w-md">
        <form method="GET" action="{{ route('admin.customers.index') }}" class="relative">
            ...
            @if(request('search'))
                <a href="{{ route('admin.customers.index') }}" class="absolute right-3 top-3 text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></a>
            @endif
        </form>
    </div>
...
@push('scripts')
<script>
    let deleteBaseUrl = "{{ route('admin.customers.destroy', ['id' => '__ID__']) }}";
    let storeUrl = "{{ route('admin.customers.store') }}";
    let updateBaseUrl = "{{ route('admin.customers.update', ['id' => '__ID__']) }}";

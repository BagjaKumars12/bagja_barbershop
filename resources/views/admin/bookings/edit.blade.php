        ...
        <div class="rounded-lg shadow p-6" :class="darkMode ? 'bg-gray-800' : 'bg-white'">
            ...
            <form method="POST" action="{{ route('admin.bookings.update', $booking->id) }}">
                ...
                <div class="mt-4">
                    ...
                    <select name="status" x-model="status" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37]"
                            :class="darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'" required>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-[#D4AF37] text-white rounded-lg hover:bg-[#b8942f] transition">Update</button>
                </div>
            </form>
        </div>
    </div>
...
@php
    ...
    $selectedServicesData = $booking->services->map(function($s) {
        return [
            'id' => $s->id,
            'name' => $s->name,
            'price' => (float) $s->price,   // konversi ke float
            'image' => $s->image ? asset('storage/services/' . $s->image) : null,
        ];
    })->values()->toArray();
@endphp
<script>
    function bookingForm() {
        return {
            ...
            selectedIds: @json($selectedServiceIds),
            selectedServices: @json($selectedServicesData),
            customerId: {{ $booking->customer_id }},
            barberId: {{ $booking->barber_id }},
            bookingTime: '{{ $booking->booking_time->format('Y-m-d\TH:i') }}',
            ...
            toggleService(id, name, price, image) {
                price = parseFloat(price);   // pastikan number

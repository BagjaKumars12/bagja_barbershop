...
<div class="space-y-6" x-data="bookingManager()">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.bookings.create') }}" class="px-4 py-2 bg-[#D4AF37] text-white rounded-lg hover:bg-[#b8942f] transition flex items-center space-x-2">
    ...
    <div class="relative max-w-md">
        <form method="GET" action="{{ route('admin.bookings.index') }}" class="relative">
            ...
            @if(request('search'))
                <a href="{{ route('admin.bookings.index') }}" class="absolute right-3 top-3 text-gray-400 hover:text-gray-600"><i class="fas fa-times"></i></a>
            @endif
        </form>
    </div>
                ...
                @forelse($bookings as $booking)
                    <tr :class="darkMode ? 'bg-gray-800' : 'bg-white'">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <a href="{{ route('admin.bookings.edit', $booking->id) }}" class="text-gray-400 hover:text-gray-600 mr-2">
                                <i class="fas fa-edit"></i>
                            </a>
                            <button @click="confirmDelete({{ $booking->id }}, '{{ addslashes($booking->booking_code) }}')" class="text-gray-400 hover:text-red-600">
...
@push('scripts')
<script>
    let deleteBaseUrl = "{{ route('admin.bookings.destroy', ['id' => '__ID__']) }}";
